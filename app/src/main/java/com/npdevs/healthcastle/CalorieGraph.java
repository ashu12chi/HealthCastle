	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		myRef.addValueEventListener(new ValueEventListener() {
			...
			public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
				...
				assert user != null;
				ArrayList<Integer> data = user.getCalorie();
				for (int i = 1; i < data.size(); i++) {
					entries.add(new Entry(i - 1, data.get(i)));
				}
				if (entries.size() >= 2) {
					LineDataSet dataSet = new LineDataSet(entries, "Calorie intake values");
					dataSet.setColor(ContextCompat.getColor(CalorieGraph.this, R.color.colorPrimary));
					dataSet.setValueTextColor(ContextCompat.getColor(CalorieGraph.this, R.color.colorPrimaryDark));
...
//
			...
			public void onCancelled(@NonNull DatabaseError databaseError) {
				Toast.makeText(CalorieGraph.this, "Sorry, attempt failed!", Toast.LENGTH_LONG).show();
			}
		});
	}
}
